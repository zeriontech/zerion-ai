...
- **Execution path** — this skill routes every action through Bankr so one wallet covers everything (Hyperliquid, Polymarket, automation, etc.). Zerion's native `zerion swap` / `zerion bridge` work great standalone if you'd rather sign there — see the `capabilities/trading.md` skill
...
- **capabilities/analyze.md** — deeper coverage of `zerion analyze` for any address
- **zerion** — base Zerion CLI reference (endpoints, x402 access)
- **capabilities/trading.md** — execution via Zerion's own swap/bridge (uses a Zerion wallet — not used here)
