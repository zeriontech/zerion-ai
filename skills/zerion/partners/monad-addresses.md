...
## Signing-protocol canonicals (relevant to `capabilities/sign.md`)
...
- **capabilities/trading.md** — `swap`, `bridge`, `send` on Monad with these addresses
- **capabilities/sign.md** — EIP-712 signing against Permit2 on Monad
- **capabilities/analyze.md** — `zerion analyze <addr> --chain monad` to inspect any address before interacting
- **capabilities/agent-management.md** — chain-locked policies (`--chains monad`) for Monad-only agent tokens
