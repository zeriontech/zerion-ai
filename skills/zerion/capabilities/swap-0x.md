...
**When to use this skill vs `capabilities/trading.md`:**
...
- Use **`capabilities/trading.md`** for quick CLI-driven swaps via `zerion swap` / `zerion bridge` on supported chains.
...
- **capabilities/trading.md** — Zerion-native swaps/bridges via `zerion swap` / `zerion bridge` (simpler, no API key needed for supported chains)
- **capabilities/analyze.md** — Check portfolio and balances before swapping (`zerion analyze <addr>`)
- **capabilities/agent-management.md** — Set up agent tokens and policies required for Zerion-signed transactions
