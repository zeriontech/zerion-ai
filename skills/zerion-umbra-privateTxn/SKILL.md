...
> migration pattern, and code examples → Advanced §3.
...
plus the live fee-provider output. See `Pitfalls §2` for the preflight
pattern.
...
See `Pitfalls §2` for the pattern.
...
`AccountNotInitialized` (pool not deployed) — see [Pitfalls §13](the Pitfalls section).
...
├─ CreateUtxoError                   retry network/RPC; insufficient SOL → Pitfalls §2/§4
├─ ClaimUtxoError                    retry — but check on-chain nullifier first (Pitfalls §3)
...
├─ EncryptedDepositError             retry; if callback dropped → getStagedSplRecovererFunction (Pitfalls §6)
...
└─ RelayerError                      `src/relayer/relayer.ts:74`  — fields: operation, statusCode, code; retry per the Relayer API section
```
...
try {
  ...
  if (e instanceof ClaimUtxoError) {
    ...
    if (e.stage === "transaction-send") {                            // VERIFY ON-CHAIN before retry — see Pitfalls §3
      // check nullifier on-chain; if burnt, the claim already landed
      throw e;                                                       // surface to caller; let Pitfalls §3 wrapper handle it
    }
  ...
  if (e instanceof CreateUtxoError) {
    ...
    //   "transaction-compile" | "transaction-sign" | "transaction-validate" |
    //   "transaction-send"
    if (e.stage === "validation")          throw e;                  // bad inputs; preflight in Pitfalls §2 prevents most of these
  ...
  if (e instanceof EncryptedDepositError) {
    ...
    // getStagedSplRecovererFunction (Pitfalls §6).
    return retryWithBackoff(someOp);
  }

  if (e instanceof FetchUtxosError) return retryWithBackoff(someOp);
  if (e instanceof IndexerError)    return retryWithBackoff(someOp);   // see the Indexer API section
  if (e instanceof RelayerError)    return handleRelayerError(e);      // see the Relayer API section
...
> **HTTP 409 with `code: "DUPLICATE_OFFSET"`** (see Relayer API),
> not via a `ClaimUtxoError.stage`. For "is this UTXO already claimed?",
> check the on-chain nullifier directly — see Pitfalls §3.
...
- `ComputationMonitorError(stage="timeout")` — check on-chain; the op may have
  succeeded already (Pitfalls §3).
- `RelayerError` with `statusCode === 409 && code === "DUPLICATE_OFFSET"` — nullifier already reserved upstream; verify on-chain before retry (see Relayer API idempotency pattern).
...
Distinguish from the Pitfalls §13 case: a 3012 / `AccountNotInitialized`
...
- `transactionForwarder: TransactionForwarder` — broadcasts and confirms transactions. Two methods:
  ...
  - **`getUmbraRelayer({ apiEndpoint })` returns a `TransactionForwarder`** — this is how relayer-paid claims plug in (see [Flows §6](the Flows section) and Relayer API).

**Persistence**:
- `masterSeedStorage` — three hooks: `load()`, `store()`, `generate()`. Full shape + the `generate` footgun in [Pitfalls §7a](the Pitfalls section). Persistence security in [Pitfalls §7b](the Pitfalls section).
...
- The on-chain `EncryptedUserAccount` PDA still exists, but its `x25519PublicKey` field references the old key. **You must re-register** so the new key gets stored on-chain (see [Flows §1](the Flows section)).
...
- High-level summary in Constants "Key rotation — `offsets` parameter".
- Master-seed derivation pipeline (which the offsets feed into) in [Flows §1](the Flows section).
...
- Registration callbacks example with `setStatus`/`setProgress` is also shown in [Flows §1](the Flows section) "Lifecycle callbacks (optional)".
- The `closeProofAccount` hook is documented (publicly invisible) — re-running a failed create handles it automatically per [Pitfalls §4](the Pitfalls section).
...
pattern) lives in [Flows §8](the Flows section). This section covers the
...
provides. See Privacy tiers — even Tier 1 (ETA → ETA) collapses
...
- Foundational ZK prover content (factory names, interface map, `IZkProverSuite`, `IZkAssetProvider` basics, performance numbers, baseline Worker pattern) → [Flows §8](the Flows section).
- Privacy implications of remote proving → Privacy tiers.
- Errors with `stage === "zk-proof-generation"` → Errors.
