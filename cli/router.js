...
function printUsage() {
  const usage = {
    ...
    wallet_management: {
      ...
      "wallet add <address|ens> --name <name>": "Add a read-only wallet (address only, no keys — signs via the web app)",
      "wallet set-review-threshold <wallet> <usd|off>": "Route transactions over <usd> to the web app for human review",
      "wallet list": "List all wallets (local + read-only)",
