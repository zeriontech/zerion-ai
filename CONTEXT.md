...
**Prepared group**:
The output of one trading command run with `--prepare`: a self-describing, **nonce-free** envelope (`{ kind, version, ecosystem, chain, address, walletName, route, summary, transactions:[…] }`) carrying that command's transaction group plus its **gate decision** (`route`). `--prepare` builds and gates but **does not execute** — it defers to a **transaction bundle**. One command = one single-chain group (`swap` → `[approve, swap]`; `consolidate` → all its approve+swap pairs as one group).
_Distinct from_: **group** — the inner unit the web app reviews/executes as one entity; a prepared group envelope wraps exactly one group.

**Transaction bundle**:
A queue of **prepared groups** collected by the `bundle` command (repeatable `--group`) and executed together. `bundle` enforces **same signer address** (cross-chain groups allowed), re-validates (policies + aggregate balance + quote freshness), then picks one route for the whole queue **strictest-wins**: any group routed to the **web-app handoff** delegates the entire queue to the web app in one link; if all groups are local, `bundle` signs+broadcasts them locally one-by-one. Always exits 0 with per-group results.
...
**Link contract**:
The URL shapes the web app owns. Transactions: `https://<host>/cli/transaction?address=<signer>#tx=<base64url(deflateRaw(JSON.stringify(payload)))>`. The `version` field discriminates two shapes the web app decodes (ADR-0003): **v1** `{ version: 1, transactions: Entry[] }` — one fully-formed single-chain group with nonces, emitted by single-command `send`/`swap`/`bridge`; **v2** `{ version: 2, groups: Entry[][] }` — an ordered queue of independent groups (nonce-free; the web app assigns pending nonces per group), emitted by `bundle`. An `Entry` is `{ evm: TransactionEVM, label? }` (EVM) or `{ solana: { raw: <base64 unsigned tx> }, label? }` (Solana). A **group** is single-chain; a v2 queue may span chains for one signer `address`. Messages: `https://<host>/cli/message?address=<signer>#msg=<same codec>`, whose payload carries exactly one **message request** — `{ kind: "personal", raw: <0x-hex bytes>, display?: <utf8 text>, chainId }` for EIP-191, `{ kind: "typedData", typedData: {domain,types,primaryType,message}, chainId }` for EIP-712, or `{ kind: "solanaMessage", raw: <0x-hex bytes>, display? }` for Solana (raw ed25519, no chainId). The `address` query param is a 0x address for EVM and a base58 pubkey for Solana. The CLI is the producer of these links; the web app is the consumer.

**Callback**:
A fire-and-forget `POST http://127.0.0.1:<port>/` the web app sends back with signing progress/result. The vocabulary is version-specific (ADR-0003). **v1** (single command): `signed { index, hash }` progress, then a terminal `completed { hashes }` / `failed { failedIndex, hashes, error }` / `rejected`. The message handoff shares the v1 terminal names but has no `signed` progress and its `completed` carries `signature` instead of `hashes`. **v2** (bundle queue): `signed { group, index, hash }` progress, one terminal **per group** (`group-completed` / `group-skipped` / `group-failed`), then a single latched `summary { groups: [{ group, outcome, hashes }] }` (`outcome ∈ completed|skipped|failed`) — the terminal the CLI resolves on, mapping `skipped → rejected`.
Production hardening (decided 2026-07-03, enforced 2026-08-03): the payload carries a **one-time nonce** the web app echoes in every callback POST; the CLI **drops any callback that doesn't carry it** — a wrong token and a missing one are treated alike, since a token-less POST is indistinguishable from a forged one (ADR-0002, ADR-0006). The first drop prints a stderr explanation, because the usual cause is a cached web-app build predating the echo and the symptom would otherwise be an unexplained timeout. On `completed` the CLI **verifies each reported hash on-chain** before reporting success (trust-but-verify).
For a **transaction bundle** (v2, decided 2026-07-22): the one-time nonce is the **only** check standing — on-chain hash verification is **relaxed** because a bundle may span chains a single client can't cover (ADR-0004).
...
- A **link contract** carries either one fully-formed transaction **group** (v1) or a **queue** of groups (v2; a group is single-chain, the queue may span chains for one signer address), or exactly one message request, plus an optional **callback** port.
...
- The **link contract**'s EVM transaction is the Zerion API's hex-string `TransactionEVM` shape (not viem's BigInt shape). The CLI fills `nonce` + `gas` but leaves `maxFee`/`maxPriorityFee`/`gasPrice` **null** — the connected wallet in the web app estimates fees at sign time. **Exception — bundle path:** a **prepared group** / **transaction bundle** omits `nonce` too (the web app assigns nonces, which is what makes cross-chain bundles and per-tx rejection work); single-command direct handoff is unchanged.
...
- **Bypass paths refuse**: a command that still cannot hand off exits with a clear error when the wallet is read-only or the value exceeds its **review threshold** — the guardrail never downgrades to a warning. (Solana `swap`/`bridge`/`send` and message signing hand off — see below; **`consolidate` now hands off via a transaction bundle** as one single-chain group, reversing its earlier refuse behavior — decided 2026-07-22, ADR-0005.)
