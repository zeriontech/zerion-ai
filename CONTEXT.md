...
Production hardening (decided 2026-07-03): the payload carries a **one-time nonce**; the CLI verifies it when the web app echoes it in a callback (mismatches ignored) and accepts it silently when absent (the web app does not echo it yet). On `completed` the CLI **verifies each reported hash on-chain** before reporting success (trust-but-verify).
For a **transaction bundle** (v2, decided 2026-07-22): the one-time nonce still applies, but on-chain hash verification is **relaxed** — the CLI trusts the reported per-group outcome because a bundle may span chains a single client can't cover (ADR-0004).
